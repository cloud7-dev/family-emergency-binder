...
- Add structured category fields for documents, medical, insurance, contacts, and recovery records / 문서, 의료, 보험, 연락처, 복구 기록에 구조화 필드 입력
- Edit existing records and their structured fields inline / 기존 기록과 구조화 필드를 카드 안에서 수정
- Mark records reviewed and track stale or expired entries / 기록 검토 완료 표시와 오래되거나 만료된 항목 추적
...
Schema v3 keeps the v2 record and attachment fields, then adds structured record foundations:

- `records[].fields` for category details such as document location, medical provider, insurance type, contact priority, and recovery location.
- `records[].fields.lastReviewedAt` for the v0.6 review workflow.
...
- Insurance records need provider and policy type.
- Contacts need name, relationship, and contact hint.
- Recovery records need service and recovery location.
- Records older than 90 days without review are marked as needs review, and expired document or insurance dates are marked stale.
...
- Edit a structured record, cancel without saving, and change category-specific fields safely.
...
- Render offline app shell from service worker cache `family-emergency-binder-v6`.
...
- Drag-and-drop, bulk ZIP export, Argon2id, cloud sync, OCR, external audit, and repo rename are out of v0.7 scope.
...
Structured fields can also contain sensitive household metadata. Keep policy numbers, account identifiers, contact details, and recovery locations as hints unless the full vault audience is intentionally trusted. / 구조화 필드도 민감한 가정 메타데이터가 될 수 있습니다. 증권번호, 계정 식별자, 연락처, 복구 위치는 전체 vault 열람자를 신뢰할 때가 아니라면 힌트 형태로 유지하세요.
...
- Backup verification flow / 백업 검증 흐름
...
`readybinder`, `emergency-binder`, `emergency-vault`, `family-emergency-binder`, `digital-vault`, `encrypted-vault`, `encrypted-attachments`, `attachment-preview`, `offline-vault`, `structured-vault`, `emergency-preparedness`, `local-first`, `privacy-first`, `offline-first`, `pwa`, `personal-data`, `preparedness`, `secure-notes`, `document-management`, `password-recovery`, `open-source`
